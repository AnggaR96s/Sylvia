...
@register(outgoing=True, pattern=r"^\.mq(?: |$)(.*)")
async def stickerchat(event):
    ...
    if reply.media:
        if reply.media.document.mime_type in ("tgsticker", "mp4"):
            await event.edit(
                "`Animated stickers and mp4 formats are not supported.`"
            )
            return
    await event.delete()
    user = (
    ...
    res, msg = await process(fetchmsg, user, bot, reply, repliedreply)
    if not res:
        return
    msg.save(".tmp/sticker.webp")
    await bot.send_file(event.chat_id, ".tmp/sticker.webp", reply_to=reply)
...
async def process(msg, user, client, reply, replied=None):
    ...
    italic = ImageFont.truetype(
        ".tmp/Roboto-Italic.ttf", 33, encoding="utf-16")
    ...
    namefallback = ImageFont.truetype(
        ".tmp/Quivira.otf", 43, encoding="utf-16")
    for letter in tot:
    ...
    textfallback = ImageFont.truetype(
        ".tmp/Quivira.otf", 33, encoding="utf-16")
    textcolor = "white"
    for line in text:
        for letter in line:
            ...
            for offset, length in bold.items():
                if index in range(offset, length):
                    font2 = ImageFont.truetype(
                        ".tmp/Roboto-Medium.ttf", 33, encoding="utf-16"
                    )
                    textcolor = "white"
            ...
            for offset, length in mono.items():
                if index in range(offset, length):
                    font2 = ImageFont.truetype(
                        ".tmp/DroidSansMono.ttf", 30, encoding="utf-16"
                    )
                    textcolor = "white"
            for offset, length in link.items():
                if index in range(offset, length):
                    font2 = ImageFont.truetype(
                        ".tmp/Roboto-Regular.ttf", 30, encoding="utf-16"
                    )
                    textcolor = "#898989"
            if letter in emoji.UNICODE_EMOJI:
                newemoji, mask = await emoji_fetch(letter)
...
async def doctype(name, size, htype, canvas):
...
CMD_HELP.update(
    {"quotemaker": ">`.mq` <reply to chat>" "\nUsage: Like QuotLy but Stonks."}
)
